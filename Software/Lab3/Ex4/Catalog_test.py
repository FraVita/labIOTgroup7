...
class Services:
    ...
    def POST(self, *uri, **params):
        ...
        if any(item for item in dict["services"] if item["serviceID"] == body["serviceID"]):
            raise cherrypy.HTTPError(409, "ServiceID already exists.")

        body["insert-timestamp"] = time.time()

        dict["services"].append(body)
    ...
    def POST(self, *uri, **params):
        ...
        for i, item in enumerate(dict["devices"]):
            if item["deviceID"]==device["deviceID"]:
                ...
                found = True
                #endpointsArd=device["endpoints"];
                #endpointsFile=dict["devices"][device]["endpoints"]
                break
        if not found:
